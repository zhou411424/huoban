...
public class SearchPlanActivity extends BaseActivity implements OnClickListener, TextWatcher, PlanEditInterface, DBOperaterInterFace {
	...
	protected void setupViews() {
		planList = (List<Plan>) getIntent().getSerializableExtra("planList");
		if (planList == null) {
		...
		mSearchPlanAdapter = new SearchPlanAdapter(this, planList);
		mSearchPlanAdapter.setOriginalValues(planList);
	...
	private void dataChanged(){
		if(mListView.getAdapter()!=null){
			if(mListView.getAdapter() instanceof SearchPlanAdapter){
				processData();
				mSearchPlanAdapter.getFilter().filter(et.getText().toString());
	...
	private void processData() {
		...
		mSearchPlanAdapter.refresh(planList);
		mSearchPlanAdapter.setOriginalValues(planList);
	}
...
//			mSearchPlanAdapter.getFilter().filter(et.getText().toString());
	...
	public void afterTextChanged(Editable s) {
		...
		if(!(mListView.getAdapter() instanceof SearchPlanAdapter)){
			mListView.setAdapter(mSearchPlanAdapter);
		}
		mSearchPlanAdapter.getFilter().filter(s);
