...
public class AlbumActivity extends BaseActivity implements OnClickListener, OnRefreshListener, DBOperaterInterFace {
	...
	private void initHeadView(LayoutInflater inflater, String user_name, String user_avatar) {
		...
		if (application.getUserId(this).equals(see_id)) {
			...
			if (Utils.stringIsNotEmpty(user_avatar)) {
				ImageLoader imageLoader = ImageLoader.getInstance();
				imageLoader.displayImage(user_avatar, ivHead,new DisplayImageOptions.Builder().showImageForEmptyUri(R.drawable.ren).showImageOnFail(R.drawable.ren).showImageOnLoading(R.drawable.ren).cacheInMemory(true).cacheOnDisc(true).build());
			}
		}
