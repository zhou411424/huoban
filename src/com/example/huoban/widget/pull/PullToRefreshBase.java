...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	static final int PULL_NOT_TO_FRESH = 0x4;
	...
	private boolean showOnloading = true;

	public void showOnloading(boolean showOnloading) {
		this.showOnloading = showOnloading;
		headerLayout.setVisibility(View.INVISIBLE);
//		TextView tv = (TextView) footerLayout.findViewById(R.id.pull_to_refresh_text);
//		tv.setTextColor(Color.WHITE);
	...
	/**
	 * Implemented by derived class to return whether the View is in a state where the user can Pull to Refresh by scrolling down.
	 * 
	...
	/**
	 * Implemented by derived class to return whether the View is in a state where the user can Pull to Refresh by scrolling up.
	 * 
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			String pullLabel = context
					.getString(R.string.pull_to_refresh_pull_labeld);
			String refreshingLabel = context
					.getString(R.string.pull_to_refresh_refreshing_labela);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			String pullLabel = context.getString(R.string.pull_to_load_more_up);
			String refreshingLabel = context
					.getString(R.string.pull_to_refresh_refreshing_label);
			String releaseLabel = context
					.getString(R.string.pull_to_load_more_down);
	...
	private boolean pullEvent() {
		...
		if (newHeight != 0) {
			if (state == PULL_TO_REFRESH && headerHeight < Math.abs(newHeight)) {
				...
				return true;
			} else if (state == PULL_NOT_TO_FRESH
					&& headerHeight >= Math.abs(newHeight)) {
				state = PULL_TO_REFRESH;
				switch (currentMode) {
				case MODE_PULL_UP_TO_REFRESH:
					footerLayout.pullReadyRefresh();
					break;
				case MODE_PULL_DOWN_TO_REFRESH:
					headerLayout.pullReadyRefresh();
					break;
				}
